...
export class TenantPurchaseDetailModalComponent {
  ...
  public productLabel(productoId: number): string {
    const p = (this.products ?? []).find(x => Number(x.producto_id) === Number(productoId));
    if (!p) return `#${productoId}`;
    const code = String(p.codigo ?? '').trim();
    const desc = String(p.descripcion ?? '').trim();
    if (code && desc) return `${code} - ${desc}`;
    return desc || code || `#${productoId}`;
  }
